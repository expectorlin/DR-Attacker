    ...
    for split in splits:
        ...
        if "/" not in split:
            if args.pretrain_attacker:
                with open('tasks/R2R/data/R2R_%s_new.json' % split) as f:
...
class Tokenizer(object):
    ...
    def __init__(self, vocab=None, encoding_length=20):
        ...
        #self.target_word_num = target_word_num
        #self.candidate_word_num = candidate_word_num
    ...
    def encode_sentence(self, sentence, max_length=None):
        ...
        sentence_tok = self.split_sentence(sentence)

        return np.array(encoding[:max_length]), sentence_tok
