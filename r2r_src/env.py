...
class R2RBatch():
    ...
    def __init__(self, feature_store, batch_size=100, seed=10, splits=['train'], tokenizer=None,
                 name=None, load_target_and_candidate_word=False):
        ...
        for split in splits:
            for item in load_datasets([split]):
                ...
                for j,instr in enumerate(item['instructions']):
                    ...
                    if load_target_and_candidate_word:
                        #get target word
                        new_item['target_word_no_repeat'] = item['target_word_no_repeat'][j]
                        #get target word position
                        new_item['target_word_position'] = item['target_word_position'][j]
                        #get candidate substitution word
                        #new_item['candidate_word'] = item['candidate_substitution_word'][j]
                        #get candidate substitution word position
                        new_item['candidate_word_position'] = item['candidate_substitution_word_position'][j]


                    if tokenizer:
                        new_item['instr_encoding'], new_item['sentence_tok'] = tokenizer.encode_sentence(instr)

                    if not tokenizer or new_item['instr_encoding'] is not None:  # Filter the wrong data
                        if load_target_and_candidate_word:
                            if len(new_item["target_word_no_repeat"]) >= 2:
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if args.pretrain_attacker:
                obs.append({
                    ...
                    # new keys
                    'target_word_position': item['target_word_position'],
                    'candidate_word_position': item['candidate_word_position'],
                    'sentence_tok': item['sentence_tok']
                })
            else:
                obs.append({
                    'instr_id' : item['instr_id'],
