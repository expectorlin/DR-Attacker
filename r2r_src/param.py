...
class Param:
    def __init__(self):
        ...
        self.parser.add_argument("--mlWeight", dest='ml_weight', type=float, default=0.)
        ...
        #### new argument ####
        self.parser.add_argument('--maxTWnumber', type=int, default=25, help="max number of target word")
        self.parser.add_argument('--maxCWnumber', type=int, default=25, help="max number of candidate word")

        self.parser.add_argument('--itersAlterNav', dest="iters_alter_nav", type=int, default=4000)
        self.parser.add_argument('--itersAlterAtt', dest="iters_alter_att", type=int, default=4000)

        self.parser.add_argument('--feedbackAttacker', dest="feedback_attacker", type=str, default='sample',
                            help='How to choose next action, one of  ``sample`` and ``argmax``')

        # training setting
        self.parser.add_argument("--pretrainAgent", dest="pretrain_agent", action='store_true',
                      help="pretrain agent")
        self.parser.add_argument("--pretrainAttacker", dest="pretrain_attacker", action='store_true', \
                        help="pretrain attacker")
        self.parser.add_argument("--advTrain", dest="adv_train", action='store_true',
                      help="adversarial training")
        self.parser.add_argument("--finetuneAgent", dest="finetune_agent", action='store_true',
                      help="finetune agent")
        self.parser.add_argument("--ifSelfSupervised", dest="if_self_supervised", action='store_true',
                      help="self-supervised reasoning task")        

        # pretrained model path
        self.parser.add_argument("--loadAttacker", dest="load_attacker", type=str, default=None)
